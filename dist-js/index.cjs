...
class EventSource {
    ...
    */
    syncSetListen(name, callback, command) {
        ...
        (async () => {
            try {
                let is_success = await core.invoke('plugin:sse|' + command, { url: this.url })
                    .then((r) => r);
                if (is_success) {
                    const unlisten = await event.listen(`${this.eventStartName}${this.url}-${name}`, (e) => {
                        callback?.(e);
                    });
    ...
    constructor(url) {
        ...
        this._onopen = null;
        this.url = url;
        this._state = exports.State.Connecting;
        this.open().then((r) => (this._state = r ? exports.State.Open : exports.State.Closed));
    }
    async open() {
        let r = await core.invoke('plugin:sse|open_sse', { url: this.url });
        if (r) {
            this._state = exports.State.Open;
        }
        return r;
    }
    /** Add named listener */
    async addEventListener(eventName, callback) {
        let is_success = await core.invoke('plugin:sse|add_event_listener', { url: this.url, name: eventName })
            .then((r) => r);
        if (is_success) {
            ...
            const unlisten = await event.listen(`${this.eventStartName}${this.url}-${eventName}`, (e) => {
                callback(e);
            });
    ...
    async removeEventListener(eventName) {
        let is_success = await core.invoke('plugin:sse|remove_event_listener', { url: this.url, name: eventName });
        if (is_success) {
    ...
    async close() {
        let r = await core.invoke('plugin:sse|close_sse', { url: this.url });
        if (r) {
            this._state = exports.State.Closed;
        }
        return r;
    }
}
